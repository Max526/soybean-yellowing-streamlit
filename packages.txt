...
def load_uploaded_image(uploaded_file: Any) -> Image.Image:
    try:
        ...
        raise ValueError(f"無法讀取圖片 {uploaded_file.name}：{exc}") from exc
...
def pil_to_bgr(image: Image.Image, resize_long_side: int) -> np.ndarray:
...
def preprocess_image(image_bgr: np.ndarray, apply_gray_world: bool, apply_clahe: bool) -> np.ndarray:
    processed = image_bgr
    if apply_gray_world:
